...
describe('CloudRedisClient', () => {
  it('has servicePath', () => {
    const servicePath = redisModule.v1beta1.CloudRedisClient.servicePath;
    assert(servicePath);
  });

  it('has apiEndpoint', () => {
    const apiEndpoint = redisModule.v1beta1.CloudRedisClient.apiEndpoint;
    assert(apiEndpoint);
  });

  it('has port', () => {
    const port = redisModule.v1beta1.CloudRedisClient.port;
    assert(port);
  ...
  it('should create a client with gRPC fallback', () => {
    const client = new redisModule.v1beta1.CloudRedisClient({fallback: true});
    assert(client);
  });
